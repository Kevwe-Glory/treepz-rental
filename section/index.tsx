...
const AboutUs = () => {
  ...
  const categorizeCars = (cars: CarProps[]) => {
    const categories = {
      Group: cars.filter(car => car.make.toLowerCase() === 'Group'),
      Basic: cars.filter(car => car.make.toLowerCase() === 'Basic'),
      luxury: cars.filter(car => car.make.toLowerCase() === 'luxury'),
      others: cars.filter(car => !['Group', 'Basic', 'luxury'].includes(car.make.toLowerCase())),
    };
    return categories;
  };
  ...
  return (
    <div className="bg-gray-50">
      ...
      <section className="container mx-auto py-16 px-6 text-center">
        <h2 className="text-4xl font-semibold text-gray-800 mb-8">Our Fleets</h2>
        ...
        {loading ? (
          ...
          <div>
            {/* Groups */}
            <h3 className="text-2xl font-semibold text-gray-800 mb-4">Groups</h3>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8 mb-8">
              {categorizedFleets.Group.slice(0, 6).map((fleet, index) => (
                <div key={index} className="bg-white shadow-lg rounded-lg overflow-hidden">
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-gray-600">Capacity: {fleet.passengerCapacity}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {/* Basic Cars */}
            <h3 className="text-2xl font-semibold text-gray-800 mb-4">Basic Cars</h3>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8 mb-8">
              {categorizedFleets.Basic.slice(0, 6).map((fleet, index) => (
                <div key={index} className="bg-white shadow-lg rounded-lg overflow-hidden">
                  <Image
                    src={fleet.image}
                    alt={`Image of a ${fleet.make} by ${fleet.make}`}
